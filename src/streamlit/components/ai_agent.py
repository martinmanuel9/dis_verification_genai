...
FASTAPI = config.endpoints.agent
FAST_API = config.endpoints.api
            ...
            if st.button("Refresh Agent List", key="manage_refresh"):
                try:
                    with st.spinner("Loading agents from database..."):
                        agent_data = api_client.get(f"{FASTAPI}/get-agents", timeout=10)
                    ...
                    try:
                        with st.spinner("Generating Word document..."):
                            ...
                            response_data = api_client.post(
                                f"{FAST_API}/doc_gen/export-agents-word",
                                ...
                                if not new_name or len(new_name.strip()) < 3:
                                    ...
                                    try:
                                        with st.spinner("Updating agent..."):
                                            api_client.put(
                                                f"{FASTAPI}/update-agent/{agent_id}",
                                                data=update_payload,
                                                timeout=30
                                            )
                            ...
                            if st.button("Confirm Deletion", type="secondary", key="delete_confirm_button"):
                                try:
                                    with st.spinner("Deleting agent..."):
                                        api_client.delete(f"{FASTAPI}/delete-agent/{agent_id}", timeout=10)
